...
5. **Workflows.** Copy `templates/ci.yml` and `templates/claude-review.yml`
   ...
   stack (the template is Laravel/Vite-flavored). Keep the principle: build
   whatever rendering/tests need BEFORE running the gates.
...
8. **GitHub.** Create the labels from `github.labels` (via `gh label create`
   or the API). Check the `CLAUDE_CODE_OAUTH_TOKEN` secret exists
   (`gh secret list`); if not, tell the user to run `claude setup-token` and
